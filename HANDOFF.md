...
Última actualización: 2026-09-23 (cloud — aviso al cerrar votación, reencuadre del avatar propio; sin migraciones pendientes).
...
- **La fórmula del puntaje vive en `lib/puntaje.ts`, no en `lib/rating.ts`** (2026-09-25). `calcularPuntaje()` es pura (no toca la base) y la usan los dos lados: el servidor para escribir `rating_events`, y el simulador de `/puntaje` en el navegador. **No reimplementar la cuenta en la UI.** Un simulador con su propia copia de la fórmula se desincroniza al primer cambio de reglas y queda mintiendo con cara de autoridad — y la gente hace cuentas con él. El refactor se verificó comparando 6.635.520 combinaciones contra la implementación vieja: 0 diferencias.
  - Los textos de `motivos` (`ganó`, `reconocimiento+ ×2`, `no jugó (1/3)`) **se guardan en la base y se filtran con `motivos @> ["ganó"]`** desde el historial. Cambiarles el texto rompe los filtros y la ficha. Para mostrarlos hay `explicarMotivo()`, que traduce sin tocar lo guardado.
  - `formatDelta()` usa 2 decimales salvo cuando el tercero no es cero, que pasa **solo** cuando el tope recortó el partido: `(0.075).toFixed(2)` da `"0.07"`, y la tarjeta mostraba `+0.07` dos renglones arriba de un texto que decía "el tope es ±0.075".
- **Pantalla `/puntaje`: reglas + simulador** (2026-09-25). Nació de un reclamo directo: nadie sabía por qué su número se movía. Las reglas estaban escritas, sí — en un comentario de `lib/rating.ts`, que no es donde alguien del club vaya a leerlas. Los umbrales los sirve `/api/puntaje` desde `app_settings`, **no van escritos a mano en la pantalla**: el día que el admin suba "pulgares por escalón" de 3 a 4, una pantalla con la regla quemada seguiría enseñando la vieja. Se entra desde la frase "⭐ Tu puntaje se gana en la cancha" del home y desde cada tarjeta de partido.
- **Movimiento de puntaje por partido** (2026-09-25, `MatchResultCard`): quién subió y quién bajó, con el renglón exacto ("ganó · 🏆 MVP · 4 👍") al lado de la cifra. Va **plegado**, con el delta propio visible en la cabecera: son 14 jugadores por partido y el historial pinta 15 partidos seguidos. Salen los que jugaron aunque queden en 0.00 (empatar y no ganar nada *es* el resultado; esconderlo parece que faltaran datos) y, de los que no jugaron, solo los que perdieron algo por inactividad — el resto sería el club entero con un 0.00 al lado. La sección no existe hasta que cierran las votaciones, porque hasta entonces no hay `rating_events`.
...
- **Lista de jugadores del panel** (`TabJugadores`, rehecha 2026-09-25): tarjetas sin botones. La fila entera abre/cierra y ahí aparece un único **"Ver ficha de X"**; **editar vive dentro de la ficha**, que es donde se ve a quién se le va a cambiar algo. Quitar los dos botones de cada renglón es lo que dejó espacio para un avatar de 52 px (antes 32), que es lo que de verdad identifica a alguien de un vistazo. **El rol no se escribe en la lista, se pinta**: la tarjeta entera lleva el color (barra lateral + fondo) — morado superadmin, naranja admin, verde jugador — y las etiquetas "SUPERADMIN"/"ADMIN" de 9 px salieron, porque decían lo mismo dos veces y le robaban la línea al nombre. Escrito aparece dentro de la ficha (Ver), que es donde se mira el detalle. Los colores viven en `lib/roles.ts` y los usan las dos pantallas: una tarjeta naranja que abriera una ficha morada sería peor que no tener color. El 👕 salió de la tarjeta (está en la ficha y en editar) y la 🔔 **solo se pinta cuando sí tiene avisos** — el 🔕 en treinta y siete filas era ruido: lo normal no necesita ícono, lo excepcional sí. Filtros arriba con su conteo: uniforme, avisos y orden por mayor puntaje (ese orden **ignora el rol** a propósito: la pregunta es quién juega mejor).
  - **`PerfilJugadorModal` pinta "Editar" también en el estado de error.** Sin eso, una ficha que no carga dejaba al admin sin ninguna forma de llegar a editar, porque la lista ya no tiene botón por fila.
...
- **Cerrar la votación a mano avisa primero** (2026-09-23): el diálogo dice qué día se cierra sola (`cierreAutomatico()` en `lib/reconocimientos.ts`, que replica la regla del cron: 00:00 del día D+2) y cuánta gente falta por votar. El admin veía el botón y asumía que si no lo tocaba no pasaba nada; cerrar temprano reparte los reconocimientos con menos votos de los que iba a haber.
...
- **Foto del partido: recorte 16:9 obligatorio** (`lib/imagen.ts`, `components/admin/RecorteFotoModal.tsx`, desde 2026-09-21). Al elegir la foto se abre un recortador con el encuadre más grande que quepa ya puesto — un toque en "Usar así" y listo; quien quiera arrastra y hace pinza. Sale **1280×720 en WebP, ~60 KB** contra los ~1–4 MB de antes (medido: 15× menos). Los dos sitios que suben foto (`CierrePartidoCard` y `TabHistorial`) pasan por el mismo modal.
  - **1280 y no 1024:** un celular tiene ~390 px CSS pero DPR 3, o sea ~1170 físicos. A 1024 la foto se vería escalada hacia arriba. La diferencia son ~30 KB por foto.
  - **WebP y no AVIF:** `canvas.toBlob('image/avif')` NO existe como encoder en Chrome ni Safari — verificado en Chromium, devuelve `image/png` en silencio. Por eso el código no se fía de que `toBlob` haya dado algo y **comprueba el `type` del blob**, con caída a JPEG. AVIF necesitaría un encoder WASM de ~1 MB.
  - **Trampa EXIF:** las fotos de celular traen bandera de rotación y los píxeles sin rotar. Se decodifica con `createImageBitmap(file, { imageOrientation: 'from-image' })`, que además es lo que mantiene las coordenadas del recorte en el mismo espacio que vio el usuario. Sin eso la foto sale acostada 90° o el recorte sale corrido. Probado en Chromium con un JPEG de orientación 6 hecho a mano.
  - **El servidor no se fía del cliente:** `api/admin/foto` bajó de 8 MB a **3 MB**. No baja a 2 para que un cliente con caché vieja alcance a terminar su subida; el mensaje de error le dice que recargue.
  - **Fuga tapada:** el nombre lleva timestamp, así que `upsert` nunca reemplazaba nada y cada resubida dejaba la anterior ocupando espacio para siempre. Ahora, después de guardar la nueva, se borran las demás de esa carpeta — lo que además limpia las huérfanas viejas a medida que se resuben. El borrado va en try/catch: que falle no invalida la subida.
  - **La limpieza conserva la foto que `partidos.foto_url` apunta AHORA**, releída de la base, no la que subió esa petición (`sobrantes()` en `lib/fotosStorage.ts`). Con dos admins subiendo al mismo partido a la vez, quedarse con "la mía" hacía que el perdedor de la carrera borrara la foto del ganador y dejara la tarjeta apuntando a un archivo inexistente. Si no hay URL vigente no se borra nada: sin saber cuál se queda, borrar es peor que dejar basura.
  - **`storage.list()` también corta, en 100, y 100 es además su valor por defecto.** Se pagina con `listarCarpeta()`. Subir el límite a un número grande habría sido el mismo error con otro número. Si una página falla, se lanza y no se borra nada — listar a medias y borrar "el resto" sería borrar archivos vivos.
  - Ambas funciones viven en `lib/fotosStorage.ts` y no en el `route.ts` porque **un archivo de ruta de Next solo puede exportar handlers y config**: cualquier otro export rompe el build, y sin exportarlas no se podían probar.
...
- **Historial con filtros** (`/historial`, `api/historial`, `lib/historial.ts`, desde 2026-09-23): selector de jugador, año, resultado (ganó/perdió/empató) y comparación con otro jugador (juntos / en contra). El botón de entrada sigue donde estaba.
  ...
  - **No se publica un porcentaje con menos de 5 partidos decididos** (`MIN_DECIDIDOS_PCT`). Medido sobre los datos reales del club (2026-09-23): un jugador con 0-1-2 saldría como **"0.0%"**, y en una pantalla que ve todo el club eso lo marca como el peor del grupo por tres partidos. Aun con 20 decididos el margen de error ronda **±10 puntos**, así que el porcentaje **nunca es un ranking**: el total del club dio 133 victorias contra 132 derrotas, o sea que casi toda la diferencia entre jugadores es ruido. Los conteos G-P-E se muestran siempre — son el dato crudo. El umbral está replicado en la consulta 6 de `auditoria_historial.sql`; si se cambia uno, cambiar el otro.
...
- **Avatares: 256 px en WebP** (`comprimirAvatar` en `lib/imagen.ts`, 2026-09-23). Antes se guardaban a 800 px en PNG con transparencia — **~500-700 KB por jugador para pintarlos a 32 px**. Con 38 miembros eso es la mitad del storage del plan gratis, y en la lista del panel se notaba: los avatares salían a medio cargar. Medido en Chromium: **518 KB → 20 KB, 26 veces menos**, conservando la transparencia del quitafondos.
  - El respaldo, si el navegador no escribe WebP, es **PNG y no JPEG**: JPEG no tiene canal alpha y rellenaría de negro el fondo que el quitafondos había quitado.
  - **Se escribe sobre el mismo objeto** (`<uid>/avatar.png`) aunque el contenido ya sea WebP. El navegador tiene `INSERT`/`UPDATE` sobre su carpeta pero **no `DELETE`** (ver `20260917_storage_limpiar_duplicadas.sql`): subirlo como `.webp` dejaría el `.png` viejo para siempre, y son ~26 MB que nadie podría borrar desde la app. Manda el content-type, no la extensión.
  - **Recompresión de los que ya estaban** (Ajustes → "Recomprimir avatares", superadmin): la compresión la hace **el navegador del superadmin** con la misma `comprimirAvatar`, y `api/admin/avatar` solo valida y escribe con la service key. En el servidor no hay canvas, y una segunda implementación del redimensionado se separaría de la primera al primer cambio. Va de a uno, no en paralelo: son ~38 descargas y subidas desde un celular.
    - **Repetible sin daño**, que es la propiedad que importa: `necesitaRecompresion()` salta lo que ya está en WebP y por debajo de 60 KB, y si el resultado no sale más chico se deja el original. Verificado en Chromium — tras la primera pasada (592 KB → 23 KB) la segunda se salta sola, y la transparencia sobrevive dos ciclos.
    - La ruta **comprueba la firma del archivo** (`\x89PNG`, `RIFF…WEBP`) y no solo el `type` del Blob, que lo pone quien sube: un HTML o un SVG declarados como `image/webp` quedarían servidos desde un bucket público.
    - `cache: 'reload'` al bajar cada avatar: sin eso se recomprimiría la copia vieja del service worker una y otra vez.
  - **"✂️ Reencuadrar" también en Mi Perfil**, sobre la foto que ya está subida: el encuadre es lo que más se falla y no tiene por qué obligar a subir la foto otra vez. Misma mecánica que el reencuadre de la foto del partido — `fetch` con `cache: 'reload'`, no pintar la `<img>` en el canvas.
  - **El avatar se recorta con guía circular** (`RecorteFotoModal` con `aspecto={1}` y `redondo`, 2026-09-23): lo que se ve dentro del círculo es exactamente lo que queda. Antes el recorte lo hacía `comprimirAvatar` al centro por su cuenta, así que a quien saliera a un lado de la foto le cortaba media cara. El modal es el mismo de la foto del partido, parametrizado — no hay dos recortadores que mantener. Después del recorte sigue pasando por el quitafondos, y por `comprimirAvatar` otra vez porque ese devuelve PNG sin comprimir.
  - `PlayerAvatar` cae al dorsal **también cuando la imagen falla al cargar** (`onError`). Antes quedaba un círculo vacío o el icono de imagen rota del navegador.
- **El pie muestra el hash del commit desplegado**, no `package.json`. `version` llevaba en 0.1.0 desde siempre porque nadie la sube a mano, así que no decía nada sobre qué está corriendo. Sale de `VERCEL_GIT_COMMIT_SHA` (7 caracteres); en local, que no existe, muestra `0.1.0-dev`.
- **Estándar de username: nombre de camiseta + número** (`pipes22`, `alexis16`, `guerrero44`). No es cosmético: **`lib/numeroCamiseta.ts` depende de él** para pintar el dorsal cuando alguien no tiene foto. Un jugador registrado sin número pierde su dorsal en toda la app y vuelve a la inicial. Al aprobar a alguien nuevo, revisar que el username siga el estándar.
  - **El username NO es clave de nada.** Inscripciones, votos, pulgares, reconocimientos y `rating_events` apuntan al uuid; el avatar vive en `<uuid>/avatar.png`. Solo se consulta por username en tres sitios y los tres son búsquedas, no vínculos: el resolver de login, el chequeo de disponibilidad al registrarse y el aviso de registro. **Por eso renombrar a alguien es seguro y no pierde historia** — y por eso "fusionar dos cuentas" casi nunca es la herramienta correcta: si la vieja no tiene datos, basta con renombrar la buena.
  - **Hay `UNIQUE (club_id, username)`**, así que para reusar un nombre hay que liberarlo antes.
  - **El panel de admin no puede renombrar a nadie**, a propósito (`editar_jugador` rechaza `username`). El único camino es que cada quien se renombre desde Mi Perfil, y solo con el ajuste `usuarios_pueden_cambiar_username` activo. Conviene volver a apagarlo después.
- **Aviso de foto de perfil** (`components/AvisoFoto.tsx`, 2026-09-23): a quien no tiene `avatar_url` le sale un aviso en el home que sube de tono con los partidos jugados — a los 3 pasa a "ya pueden elegirte la fota". **Molesta pero NO bloquea**, a propósito: impedirle inscribirse a alguien por no tener foto deja a una persona sin jugar por un avatar, y el chiste se acaba el primer viernes que pase. Tampoco se puede cerrar: quien lo cierra una vez no lo vuelve a ver y nunca sube la foto. Desaparece solo cuando hay foto. Los nombres del "edit" están en la constante `EDITORES`, una sola línea.
- **Lista de jugadores del panel, agrupada por equipo** (`ListaJugadores` en `TabHistorial`, 2026-09-23): rejilla compacta con avatar/dorsal, separada en 🤍 blancos / 🖤 negros / 💜 morados / sin equipo / en espera. Antes era una fila por persona con su propio `REMOVER`: catorce filas altas, catorce botones rojos para algo que casi nunca se hace (y fácil de tocar sin querer al desplazar), y el estado "confirmado" repetido catorce veces sin decir nada. Agregar y remover viven **abajo, en un solo control con dos modos**. El color de equipo sale de `equipo_jugadores` + `equipos`, que ya estaba guardado y no se estaba usando.
  - **`flex`, no `grid`** (corregido 2026-09-23): con una rejilla de columnas iguales, en pantalla ancha cada nombre se estiraba a un cuarto de la fila — mucho vacío alrededor de diez caracteres. Con `flex-wrap` cada ficha mide lo que mide su nombre.
  - **La foto no lleva tope de ancho.** Estaba capada a 340 px mientras la lista de nombres ocupaba el ancho completo: al revés de lo que conviene, porque la foto es lo único de esa pantalla que gana con el espacio.
  - **La foto va primero** en el panel, antes que los nombres y el marcador: es lo que el admin quiere ver al abrir un partido, y estaba de últimas.
  - **Las fichas llevan `maxWidth: 720`**, que es lo que fija cuántas caben por fila: con fichas de ~165 px salen cuatro, y con siete por equipo eso son dos líneas parejas (4+3) en vez de una fila larga cortada donde caiga.
  - **Agregar/remover arranca plegado**, los dos botones en gris; el selector aparece solo al tocar uno y vuelve a plegarse al retocarlo. Es una acción ocasional y no tiene por qué ocupar sitio cada vez.
  - **El día de la semana va en verde**: es lo que separa un partido del siguiente en una lista larga, y en gris se perdía.
  - **Con el marcador ya guardado, el formulario no se muestra**: sale el dato y un "editar" gris y chico. Dos campos grandes y un botón verde pedían clic para algo que ya estaba hecho. Sin resultado, el formulario sale directo — ahí sí es la acción que toca.
- **El dorsal sale del username** (`lib/numeroCamiseta.ts`, 2026-09-23). La convención del club es que el username termina en el número: `alexis16` → 16, `guerrero44` → 44. **No hay columna en la base y no hace falta.** `PlayerAvatar` lo usa cuando el jugador no tiene foto, en lugar de la inicial: había **cinco usernames que empiezan por "j"** (`jbravo30`, `juli9`, `jj21`, `jhonsito23`, `jsanchez17`), así que la inicial no distinguía a nadie. Quien no lleva número (`pipesolarte`) sigue viendo su inicial. Solo acepta una o dos cifras finales y mira la racha completa de dígitos, para que un `user2024` no se convierta en el "24". Sin lookbehind en la regex: no existen en Safari anterior a 16.4. Probado contra los 25 usernames reales.
- **Reconocimientos de la tarjeta, en rejilla** (`MatchResultCard`, 2026-09-23): emoji + avatar/dorsal + nombre + categoría + punto del color de equipo, en `grid` con `auto-fill minmax(150px, 1fr)`. Con cinco reconocimientos pasó de **282 px a 120 px** (medido en Chromium; a 320 px baja sola a una columna y da 204 px, sin scroll horizontal en ningún ancho).
  - **En un empate se muestran TODOS los ganadores, no "+1"** (2026-09-23): el empate es la gracia del asunto, y esconder a uno detrás de un contador se lee como si hubiera un solo ganador. Los avatares se superponen para ocupar poco.
  - **`minmax(215px)` y no 150**: a 150 se mochaban los nombres y las categorías ("mendieta…", "Mejor Porter…"), que es justo lo que hay que leer. Menos columnas pero enteras se lee mejor que muchas cortadas.
  - **Se conserva el nombre de la categoría a propósito.** Una versión de solo emojis medía 53 px, pero "La más perrota", "El salao" y "Tronco" son el chiste del grupo y con solo el icono la tarjeta era una adivinanza.
  - **El punto de equipo NO usa el color literal:** un punto negro sobre fondo negro no se ve. El "negro" va gris medio con anillo claro. Se vio al renderizarlo, no leyendo el CSS.
  - El color sale de `equipos.color` + `equipo_jugadores`, en **una** consulta para toda la página (`coloresPorJugador`), no una por partido.
  - `src/app/page.tsx` tenía su propia `interface Badge`, copia de `MatchBadge` que ya se había quedado sin `votos` ni `avatar_url`: cargaba datos que la tarjeta no veía. Ahora es `type Badge = MatchBadge`.
