...
- **Avatares: 256 px en WebP** (`comprimirAvatar` en `lib/imagen.ts`, 2026-09-23). Antes se guardaban a 800 px en PNG con transparencia — **~500-700 KB por jugador para pintarlos a 32 px**. Con 38 miembros eso es la mitad del storage del plan gratis, y en la lista del panel se notaba: los avatares salían a medio cargar. Medido en Chromium: **518 KB → 20 KB, 26 veces menos**, conservando la transparencia del quitafondos.
  ...
  - **Esto solo arregla las subidas NUEVAS.** Los avatares ya guardados siguen pesando lo mismo. Recomprimirlos necesita una ruta de servidor con la service key, porque desde el navegador cada quien solo puede escribir en su propia carpeta.
